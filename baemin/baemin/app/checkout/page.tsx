...
export default function Home() {
    ...
    const handleSubmit =()=>{
        axios.post('http://localhost:3300/order/create-order',detail).then((res) => { 
            toast("Wow so easy!")
        setTimeout(() => {
            route.push('/statusorder')
        }, 1500);
         })
    }
    ...
    useEffect(() => { 
            ...
            try {
                const storedItems = localStorage.getItem("cart_item"); 
                if (storedItems) {
                    const parsedItems = JSON.parse(storedItems); 
                  
                    setDetail(parsedItems); 
     ...
     useEffect(() => {
        const newTotal = detail.reduce((sum, item:any) =>  (item.total_price || 0), 0);
        setTotal(newTotal);
    }, [detail]);
        ...
        <ToastContainer />
            ...
            <div className="px-16 flex flex-col gap-3 ">'
                ...
                <div className="w-full  flex flex-col bg-white rounded-md  pt-5 gap-3">
                    ...
                    <div className="text-sm mr-5">₫{total}</div>
                </div>
                        <div className="flex justify-between w-[30%] ">
                    ...
                    <div className="w-full border-t  flex flex-row justify-between items-center  pt-4  gap-4 mb-4">
                        ...
                        <div  className="w-[30%] pl-48 ">
                            <button onClick={handleSubmit} className="p-1 bg-beamin text-white w-36 rounded-md h-10 hover:brightness-105">
                                Đặt hàng
                            </button>
                        </div>
                    </div>
